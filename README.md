...
AMATELUSプロトコルは、分散型識別子（DID）、検証可能資格情報（VC）、およびゼロ知識証明（ZKP）を統合した**暗号学的身元確認メカニズム**である。

#### AMATELUSが提供するもの

AMATELUSは以下の機能を提供する：
- 公開鍵基盤に基づく分散型身識別子（DID）
- ゼロ知識証明による属性所有権の検証
- クレーム発行・保存のための検証可能資格情報（VC）

#### AMATELUSが提供しないもの

AMATELUSは以下を提供**しない**：
- **中央ディレクトリサービス**: DID解決は提示されたDIDDocumentから直接行われる
- **メディエーション・中継インフラ**: エンドポイント発見やメッセージ配信は各サービス提供者の責任
- **ユーザー認可決定**: 本人確認後の認可判定はサービス提供者が行う
- **通信エンドポイント管理**: serviceEndpoint はAMATELUSの責任範囲外

#### 実装パラダイム

AMATELUSはウォレットアプリケーションとして、以下のコンテキストで使用される：

1. **サービス駆動モデル**（一般的）
   - サービス提供者のアプリケーション（銀行、SNS、投票システム等）が起動
   - ウォレットがアプリからOSレベルのインテント/デープリンク経由で呼び出される
   - ウォレットがDIDCommメッセージをサービス提供者へ送信（HTTPS等）
   - サービス提供者が検証・認可・サービス実行

2. **物理的対面モデル**（補助的）
   - BLE（Bluetooth Low Energy）等により物理的近接性で発見
   - 公共サービス（自治体窓口）や施設入場ゲート等
   - 同じローカルネットワークでのセッション完結
...
- **サービス提供者の信頼性**: サービス提供者（銀行、政府、SNS等）の中央集権的サーバーの暗号学的・運用的信頼性を前提とする
- **通信セキュリティ**: DIDCommメッセージのトランスポート層セキュリティ（HTTPS/TLS等）はサービス提供者により提供される
- **ウォレット実装セキュリティ**: ウォレットアプリケーションの実装セキュリティ（秘密鍵管理等）は端末のセキュリティに依存する
...
DIDDoc := {id: DID, publicKey: PK, metadata: M}
```
ここで、`H`は耐衝突ハッシュ関数、`PK`は公開鍵、`M`はメタデータである。

**重要な設計判断: serviceEndpoint の廃止**

従来のDID仕様ではDIDDocumentに`serviceEndpoint`が含まれていた。しかし、AMATELUSの実装パラダイム分析により、以下の判断に至った：

- **did:amtはローカル解決のみ**: DID文字列からDIDDocumentを解決できないため、Holderが明示的にDIDDocumentを提示する
- **エンドポイント情報は送信側に既知**: ウォレットはサービス提供者のアプリから呼び出されるため、エンドポイント（HTTPS URL等）はアプリに既知
- **エンドポイント寿命の問題**: serviceEndpoint をDIDDocumentに含めると、エンドポイント契約管理がHolderに負担になる
- **結論**: did:amtではsendpointEndpointを**廃止**し、公開鍵のみをDIDDocumentに含める

これにより、以下が実現される：
- DIDの寿命 = 長期的（公開鍵ベース）
- エンドポイントの寿命 = セッションスコープ（サービス提供者管理）
- 個人の契約管理負担 = ゼロ
...
本開発過程で、AMATELUSの本質的な位置づけが明確になった：

**AMATELUS は暗号学的身元確認メカニズムであり、通信インフラストラクチャではない**
...
- **サービス駆動**: ウォレットはサービス提供者のアプリからのみ呼び出される
- **ステートレス**: セッション完結後、エンドポイント情報は保持しない
- **信頼の一元性**: 暗号学的信頼の出所はAMATELUS、ユーザー認可決定の出所はサービス提供者

### 10.3 実用的含意

これらの形式的証明と設計明確化により、AMATELUSプロトコルは以下を保証する：

- **暗号学的安全性**: 設計思想通りの動作が数学的に保証される
- **攻撃耐性**: 攻撃ベクトルが明確に特定され、対策が講じられている
- **スケーラビリティ**: 分散性により線形スケーリングが理論的に裏付けられている
- **量子耐性**: PQC採用により将来的な技術発展に対応可能
- **汎用性**: 特定の国民識別制度に依存せず、多様な社会システムで運用可能
- **インフラ非依存性**: 外部インフラの障害や検閲に耐性を持つ
- **実装明確性**: 設計責任分界が明確であり、実装判断の指針となる

### 10.4 今後の研究方向

- 具体的なZKP回路の最適化と形式検証
- 実装レベルでのサイドチャネル攻撃対策
- サービス提供者向けの実装ガイドライン策定
- 異なる社会コンテキスト（投票、医療、金融）での運用ガイドライン開発
- ウォレット実装のセキュリティ基準策定

### 10.5 社会的意義

AMATELUSプロトコルは、以下の社会的課題の解決に貢献する：

1. **プライバシー保護**: 複数DID、ZKP、一階層信頼制限により、個人情報の最小開示が実現
2. **信頼の分散化**: 公開鍵基盤により、特定の中央権力への依存を低減
3. **包摂性**: 複数DID保有可能な設計により、様々な社会的背景を持つ個人に対応
4. **監査可能性**: 匿名ハッシュ識別子により、適切に制限された監査を実現

AMATELUSプロトコルは、形式的証明に基づく堅牢な理論的基盤と明確な設計責任分界を持つ暗号学的身元確認メカニズムとして、**分散型デジタルガバナンスとサービス提供者の中央集権的信頼の最適な組み合わせ**を実現する重要な基盤技術を表している。
